...
int main()
{
    ...
    double imrow=patterns[0].im.rows;
    double imcol=patterns[0].im.cols;

    const double pxy=0.5*(imrow+imcol);

    // create a camera model (Perspective or Distortion)
    // default parameters should be guessed from image dimensions
    PerspectiveCamera cam(pxy,pxy,0.5*imcol,0.5*imrow);   // not a very good guess
